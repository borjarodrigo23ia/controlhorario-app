...
export default function CentersPage() {
    ...
    const handleDelete = async (id: number) => {
        if (!confirm('┬┐Est├ís seguro de eliminar este centro?')) return;
        try {
            const token = localStorage.getItem('dolibarr_token');
        ...
        if ('geolocation' in navigator) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    toast.success('Ubicaci├│n detectada');
                },
                (error) => toast.error('Error al detectar ubicaci├│n')
            );
        } else {
            toast.error('Navegador no soporta geolocalizaci├│n');
        }
    };
                    ...
                    <div className="bg-white rounded-[2rem] w-full max-w-lg max-h-[85vh] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in duration-300">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar px-6 py-5 space-y-5">
                            ...
                            <div className="space-y-2">
                                <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest ml-1">Tipo de Ubicaci├│n</label>
                    ...
                    <div className="flex flex-col items-center text-center space-y-5">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <p className="text-xs text-gray-500 font-medium max-w-[280px] leading-relaxed">
                                <span className="text-gray-900 font-bold">{conflictData?.user.firstname} {conflictData?.user.lastname}</span> ya tiene otro/s centro/s asignado/s. ┬┐Deseas proceder con la asignaci├│n?
                            </p>
                        </div>
                        ...
                        <div className="w-full pt-1">
                            ...
                            >
                                <span>Continuar asignaci├│n</span>
                                <ArrowRight size={14} />
                            </button>
                        </div>
                    </div>
